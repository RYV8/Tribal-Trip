...
export class ApiError extends Error {
  constructor(
    message,
    { status = 0, code = "api_error", payload = null } = {},
  ) {
    super(message);
    this.name = "ApiError";
    this.status = status;
...
function getApiBaseUrl() {
  const runtimeApiUrl =
    typeof window !== "undefined" ? window.TRIBE_TRIP_CONFIG?.apiUrl : "";
  return (
    runtimeApiUrl ||
    ...
    try {
      ...
      throw new ApiError(
        timedOut
          ...
          : "Unable to reach the Tribe Trip API. Check your connection and retry.",
...
export function createLocalSecret(token, payload) {
  return fetchJson("/local-secrets", {
    method: "POST",
    token,
    body: payload,
    timeoutMs: 5000,
...
export function createAdminLocation(token, payload) {
  return fetchJson("/admin/locations", {
    method: "POST",
    token,
    body: payload,
    timeoutMs: 5000,
...
export function getAdminLocations(token, status = "all") {
  return fetchJson(`/admin/locations?status=${encodeURIComponent(status)}`, {
    token,
    timeoutMs: 5000,
  });
}
...
export function updateAdminLocation(token, slug, payload) {
  return fetchJson(`/admin/locations/${encodeURIComponent(slug)}`, {
    method: "PUT",
    token,
    body: payload,
    timeoutMs: 5000,
...
export function createAdminStory(token, payload) {
  return fetchJson("/admin/stories", {
    method: "POST",
    token,
    body: payload,
    timeoutMs: 5000,
...
export function getAdminStories(token, status = "all") {
  return fetchJson(`/admin/stories?status=${encodeURIComponent(status)}`, {
    token,
    timeoutMs: 5000,
  });
}
...
export function updateAdminStory(token, slug, payload) {
  return fetchJson(`/admin/stories/${encodeURIComponent(slug)}`, {
    method: "PUT",
    token,
    body: payload,
    timeoutMs: 5000,
...
export function createAdminArtifact(token, payload) {
  return fetchJson("/admin/artifacts", {
    method: "POST",
    token,
    body: payload,
    timeoutMs: 5000,
...
export function getAdminArtifacts(token, status = "all") {
  return fetchJson(`/admin/artifacts?status=${encodeURIComponent(status)}`, {
    token,
    timeoutMs: 5000,
  });
}
...
export function updateAdminArtifact(token, slug, payload) {
  return fetchJson(`/admin/artifacts/${encodeURIComponent(slug)}`, {
    method: "PUT",
    token,
    body: payload,
    timeoutMs: 5000,
...
export function uploadAdminMedia(token, file, metadata = {}) {
  ...
  if (metadata.sourceCredit)
    formData.append("sourceCredit", metadata.sourceCredit);
...
export function getAdminAuditLogs(token, limit = 50) {
  return fetchJson(`/admin/audit-logs?limit=${encodeURIComponent(limit)}`, {
    token,
    timeoutMs: 5000,
  });
}
...
export function updateAdminSource(token, id, payload) {
  return fetchJson(`/admin/sources/${encodeURIComponent(id)}`, {
    method: "PUT",
    token,
    body: payload,
    timeoutMs: 5000,
...
export function createAdminCountry(token, payload) {
  return fetchJson("/admin/countries", {
    method: "POST",
    token,
    body: payload,
    timeoutMs: 5000,
...
export function updateAdminCountry(token, slug, payload) {
  return fetchJson(`/admin/countries/${encodeURIComponent(slug)}`, {
    method: "PUT",
    token,
    body: payload,
    timeoutMs: 5000,
...
export function updateAdminCultureGuide(token, countrySlug, payload) {
  return fetchJson(`/admin/culture-guides/${encodeURIComponent(countrySlug)}`, {
    method: "PUT",
    token,
    body: payload,
    timeoutMs: 8000,
...
export function getAdminLocalSecrets(token, status = "submitted") {
  return fetchJson(
    `/admin/local-secrets?status=${encodeURIComponent(status)}`,
    { token },
  );
}

export function updateAdminLocalSecretStatus(token, id, status) {
  return fetchJson(`/admin/local-secrets/${id}`, {
    method: "PATCH",
    token,
    body: { status },
  });
...
export async function safeFetchJson(path, opts) {
  ...
  try {
    ...
    console.warn(
      "safeFetchJson: non-JSON response for",
      path,
      "status",
      res.status,
    );
