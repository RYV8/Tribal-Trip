...
test.after(async () => {
  if (uploadedPublicId)
    fs.rmSync(path.join(__dirname, "..", "uploads", uploadedPublicId), {
      force: true,
    });
...
test("request validation rejects malformed write payloads", async () => {
  ...
  assert.ok(
    invalidRegister.payload.details.some((detail) =>
  ...
  const invalidStatus = await request("/api/admin/locations?status=live", {
    token: editorToken,
  });
...
test("admin routes require an editorial role", async () => {
  ...
  const normalUser = await request("/api/admin/locations", {
    token: userToken,
  });
...
test("editor can prepare review records but cannot publish, archive, or read audit logs", async () => {
  ...
  const archived = await request(
    `/api/admin/locations/${created.payload.data.slug}`,
...
test("admin can archive records and read audit logs", async () => {
  const created = await request("/api/admin/locations", {
    ...
    body: {
      ...
      history:
        "This record verifies admin archive permissions and audit logging.",
      status: "review",
    },
  });
  assert.equal(created.status, 201);
...
test("local secret moderation respects editor and admin boundaries", async () => {
  ...
  const review = await request(
    `/api/admin/local-secrets/${submitted.payload.data.id}`,
    {
      method: "PATCH",
      token: editorToken,
      body: { status: "review" },
  ...
  const editorPublish = await request(
    `/api/admin/local-secrets/${submitted.payload.data.id}`,
    {
      method: "PATCH",
      token: editorToken,
  ...
  assert.ok(
    publicSecrets.payload.data.some(
      (item) => item.title === "Quiet courtyard test",
    ),
  );
});
...
test("media upload rejects non-images and accepts allowed image types", async () => {
  ...
  const badUpload = await request("/api/admin/media", {
    method: "POST",
    token: adminToken,
    body: badForm,
  });
  ...
  const goodUpload = await request("/api/admin/media", {
    method: "POST",
    token: adminToken,
    body: goodForm,
  });
...
test("content sources are editorial-managed and visible on public catalogue records", async () => {
  ...
  const listed = await request(
    "/api/admin/sources?itemType=location&itemId=published-test-museum",
    { token: editorToken },
  );
  assert.equal(listed.status, 200);
  assert.ok(
    listed.payload.data.some((source) => source.id === created.payload.data.id),
  );

  const publicLocations = await request("/api/locations");
  assert.equal(publicLocations.status, 200);
  const published = publicLocations.payload.data.find(
    (location) => location.id === "published-test-museum",
  );
  assert.ok(
    published.sources.some(
      (source) => source.title === "Museum catalogue note",
    ),
  );

  const updated = await request(
    `/api/admin/sources/${created.payload.data.id}`,
    {
      method: "PUT",
      token: editorToken,
...
test("country and culture guide administration respects editorial boundaries", async () => {
  ...
  const reviewGuide = await request("/api/admin/culture-guides/testland", {
    ...
    body: {
      ...
      musicStyles: [
        {
          name: "Test drums",
          description: "Community percussion.",
      ...
      clothingStyles: [
        {
          name: "Test cloth",
          description: "Formal woven cloth.",
  ...
  const editorPublishedEdit = await request(
    "/api/admin/culture-guides/testland",
    {
      method: "PUT",
      token: editorToken,
...
test("admin user management lists users, limits role changes, and blocks disabled accounts", async () => {
  ...
  assert.equal(listed.status, 200);
  const managed = listed.payload.data.find(
    (user) => user.email === "managed@test.local",
  );
  assert.ok(managed);
